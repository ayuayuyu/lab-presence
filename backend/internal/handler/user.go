...
func listUsers(db *sql.DB, w http.ResponseWriter) {
	rows, err := db.Query(`SELECT id, name, student_id, created_at FROM users ORDER BY id`)
	if err != nil {
	...
	for rows.Next() {
		var u model.User
		if err := rows.Scan(&u.ID, &u.Name, &u.StudentID, &u.CreatedAt); err != nil {
...
func createUser(db *sql.DB, w http.ResponseWriter, r *http.Request) {
	...
	var u model.User
	err := db.QueryRow(
		`INSERT INTO users (name, student_id) VALUES ($1, $2) RETURNING id, name, student_id, created_at`,
		req.Name, req.StudentID,
	).Scan(&u.ID, &u.Name, &u.StudentID, &u.CreatedAt)
	if err != nil {
